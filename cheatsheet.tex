...
\DeclareMathSizes{5.8}{5.8}{5.6}{5.6}
\DeclareMathSizes{5.6}{5.6}{5.6}{5.6}
...
\newcommand{\bx}[2]{\par\vspace{2.2pt}\noindent\textbullet\ #1\hfill{\fontsize{5.6}{6}\selectfont #2}\par\nobreak}
\newsavebox{\eqbox}
\newcommand{\eq}[1]{\sbox{\eqbox}{{\fontsize{7.5}{9}\selectfont\boldmath\(\displaystyle #1\)}}\ifdim\wd\eqbox>\linewidth\typeout{WIDE-EQ at line \the\inputlineno: \the\wd\eqbox}\fi \par\nobreak\vspace{1.2pt}{\fontsize{7.5}{9}\selectfont\boldmath\centering\(\displaystyle #1\)\par}\nobreak\vspace{1.2pt}}
...
\newsavebox{\colbox}
\newenvironment{col}[1]{\begin{lrbox}{\colbox}\begin{minipage}[t]{55mm}\fontsize{5.8}{6.55}\selectfont\RaggedRight\sect{#1}}{\end{minipage}\end{lrbox}\typeout{COLUMN-HEIGHT: \the\ht\colbox + \the\dp\colbox}\usebox{\colbox}}
...
\begin{document}
\head{Probability, Markov chains and first-step analysis}{Lectures 1--5 / Tutorials 1--5\quad 1 of 2}
\noindent
\begin{col}{1\quad Probability tools}
\bx{Shared notation}{L1--L5}
\sy{X_n}{state random variable at time/step $n$.}
\sy{S}{state space: all possible states.}
...
\sy{\ind_A}{indicator: 1 if event $A$ occurs, otherwise 0.}
...
\Var(X)=\E[X^2]-(\E[X])^2\\
\E[a+bX]=a+b\E[X]\\
\Var(a+bX)=b^2\Var(X),\quad \E[\ind_A]=\PP(A)
\end{gathered}}
...
\use{Linearity needs no independence. Independent variables have zero covariance and $\E[\prod_r X_r]=\prod_r\E[X_r]$. Zero covariance does not imply independence. To verify discrete independence, check joint PMF equals product of marginals for every value combination.}
...
\end{col}\gap%
\begin{col}{2\quad Build and verify a chain}
\bx{Process and information}{L1:7; L4:5--6,26}
\eq{\{X_n:n\ge0\},\qquad \F_n=\sigma(X_0,\ldots,X_n)}
\sy{\Omega}{sample space: all complete outcomes.}
\sy{\omega}{outcome: one complete realisation.}
...
\mng{Fix $\omega$ for a sample path; fix $n$ for a random variable.}
\use{Classify time and state separately: countable means discrete. A filtration is increasing sigma-algebras, $\F_n\subseteq\F_{n+1}$. Each contains $\Omega$ and is closed under complements/countable unions.}
...
\ck{(1) Specify the state and its possible values. (2) Condition on a possible history ending at $i$. (3) Derive the full next-state law and show it depends only on $i$ (and possibly $n$). Independence of fresh randomness from the history justifies this reduction.}
...
\end{gathered}}
\sy{\pi_0}{initial distribution: row vector on the stated state order.}
\sy{P=(p_{ij})}{transition matrix: row = from, column = to.}
\mng{Initial probabilities and one-step rules specify a homogeneous chain.}
...
\end{col}\gap%
\begin{col}{3\quad Finite-time probabilities}
...
\mng{Multiplies the initial probability and successive transition factors.}
...
\mng{Weights possible past/intermediate states by evidence from the endpoint.}
\use{``Given where it ends, where was it?'' Denominator must be positive; probabilities over the unknown state sum to 1. For inhomogeneous chains, use $P^{a,t},P^{t,b},P^{a,b}$.}
...
\sy{r_{n+1}(j)}{predicted state probability before the new observation.}
\sy{z_{n+1}}{probability of the new observation given earlier evidence.}
\mng{Transitions predict the state; emissions update it using evidence.}
...
\end{col}\gap%
\begin{col}{4\quad Models and random walks}
...
\use{Only for the tutorial rule: no service when initially empty; no arrival when initially full. Other event orders need re-derivation.}
...
\sy{d}{dimension of a simple symmetric lattice walk from the origin.}
...
\mng{Scores each possible hidden path using transitions and emissions.}
...
\end{col}\gap%
\begin{col}{5\quad First-step analysis}
...
\mng{Adds the current contribution, then averages the remaining total.}
...
\end{col}
\newpage
\head{Stopping, martingales, generating functions and branching}{Common symbols continue from page 1\quad 2 of 2}
\noindent
\begin{col}{6\quad Verify and build martingales}
...
\mng{Given current information, the next value has the current mean.}
...
\sy{M_n}{square martingale: subtract accumulated increment variance.}
...
\end{col}\gap%
\begin{col}{7\quad Stopping and optional stopping}
...
\mng{By time $n$, the observed history tells whether stopping has occurred.}
\ck{Express $\{T\le n\}$ using only history events. For $T=\inf\{r\ge r_0:C_r\}$ with $C_r\in\F_r$, it is $\bigcup_{r=r_0}^{n}C_r$ (empty if $n<r_0$). A fixed deadline uses $T\wedge K$.}
\use{Hitting levels, consecutive decreases, capped observation. To disprove, exhibit two possible paths identical through $n$ but disagreeing on $T\le n$. ``Last visit'' generally needs future information.}
...
\mng{Stopping preserves the mean, provided the passage to $T$ is justified.}
...
\use{Find stopped expectation, exit probabilities or duration. Finite $T$ a.s. alone is insufficient. A bound must hold uniformly in $n$.}
...
\use{Apply OST separately to $X_n$ and $X_n^2-n\sigma^2$, checking conditions for each. With more than two exit values, the mean equation alone does not determine all probabilities; use first-step equations.}
\end{col}\gap%
\begin{col}{8\quad Generating functions and sums}\fontsize{5.6}{6.3}\selectfont
...
\mng{Stores all probabilities as polynomial/power-series coefficients.}
...
\mng{Derivatives at 1 give factorial moments, then ordinary variance.}
...
\mng{Recognises a distribution from its PGF and supplies its moments.}
...
\use{MGF questions; sum/product needs independence; random sum uses the assumptions above. Existence near 0 ensures moments and distribution identification. PGF moments use 1; MGF moments use 0.}
\end{col}\gap%
...
\use{Find $\PP(Z_n=j)=[s^j]G_n(s)$; for 0 or 1 evaluate $G_n(0)$ or $G_n'(0)$. Composition is not an ordinary power; ancestor counts give powers.}
...
\mng{Converts individual reproduction moments into population moments.}
...
\mng{Chooses the smallest PGF fixed point; survival probability is $1-q^k$.}
...
\use{``Eventually dies out'' or a threshold. Critical extinction a.s. does not imply a bounded extinction time or finite mean time. If $q<1$, $\PP_k(T=\infty)=1-q^k$ and $\E_kT=\infty$.}
\end{col}\gap%
...
\sy{m_n,s_n^2}{mean and variance of that generation's offspring law.}
\sy{a_n,v_n}{population mean and variance, initially $1,0$.}
\mng{Conditions on the current generation using its reproduction law.}
\use{Changing environments: new offspring PGF goes inside the current population PGF. Do not reverse composition order. For $k$ ancestors, start with $G_0(s)=s^k$, $a_0=k,v_0=0$.}
...
\mng{The even-generation population is an ordinary branching process.}
...
\use{Apply branching formulas with modified moments. Mean tends to 0 iff $am<1$; with positive zero-offspring probability, extinction is certain iff $am\le1$. For $m>0$: $v>1-1/m$ versus $v\ge1-1/m$, with $v\in[0,1]$.}
